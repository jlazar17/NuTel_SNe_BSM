...
\newcommand{\YY}[1]{\color{red}#1\color{black}}
...
\begin{figure}[t!]
    ...
    \caption{\textbf{The decay geometry for Majorons produced in supernova.}}
    \label{fig:geometry}
\end{figure}
...
We will consider a supernova event that happens in the galaxy at a distance $D_{\rm SN}=\unit[10]{kpc}$, which is not unlikely~\cite{Reed:2005en,Rozwadowska:2020nab}. To get the characteristic value of $\delta t$, we can take $L_1 = L_\phi$ with $L_\phi = (E_\phi/m_\phi) \Gamma^{-1}_\phi \beta$ being the decay length of $\phi$.
...
%However, at small values of $E_\phi$, the main contribution to $\delta t$ comes from the first term due to the slow-moving Majorons, while at larger values of $E_\phi$, from the second term due to a larger detour given the increase of $L_\phi$ over the decrease of $(1-\cos\alpha)$.
...
\begin{figure}[t!]
    ...
    %The sensitivities shown as solid lines are computed from this work, whereas the dashed lines are computed following~\cite{Brdar:2023tmi}.
    Additionally, we also show the reaches by supernova cooling and by the lack of observation of high-energy neutrinos from SN1987A as black lines. %\cite{Fiorillo:2022cdq}, with the width of the shading region quantifying uncertainties from supernova modeling.
    }
    \label{fig:sensitivity}
\end{figure} 
Following our strategies above, we obtain the expected exclusion limits for various BSM scenarios from no observed excess of hits on top of background noise and standard neutrino flux.
The expected exclusion limits are presented in \cref{fig:sensitivity} for the Majoron case considering both IceCube and IceCube-Gen2. To illustrate the broad physics application of the analysis, we also show results for the sterile neutrino case with magnetic portal in the Supplemental Materials.
Current limits \cite{Fiorillo:2022cdq} from the energy loss requirement, and non-observation of high-energy supernova neutrinos from SN 1987A are also shown in \cref{fig:sensitivity} for the Majoron case. With future supernova explosions, IceCube can improve current limits by one order of magnitude. 
...
JL acknowledges support from the Belgian American Educational Foundation.
...
\begin{figure}[hb!]
    ...
    The shaded regions are excluded by the combination of energy loss requirement, non-observation of photon and neutrino signals from SN1987A studied in \cite{Brdar:2023tmi} and by constraints on the energy release from supernova explosion (SNIIP explosion)\cite{PhysRevLett.128.221103, chauhan2024probing}.
    }
